...
def list_to_common_list(q_list):
    next_dict = Counter([ lx for __, lx in q_list])
    ...
    for p1 in paper_list:
        ...
        for p2 in paper_list:
            ...
            for p in paths:
                ...
                ps = p[1:-1]
                for node in ps:
                    if (my_config.REVIEW_CHECK == True):
                        try:
                            if (gg.nodes[node]['review'] == True):
                                found = True
                        except:
                            gnode = gg.nodes[node]
                            gnode['review'] = find_review(get_abstract(node))
                            if (gnode['review'] == True):
                                #print("-", gnode)
                                found = True
                    else: # Pass! result
                        found = True
...
if __name__ == '__main__':
    ...
    while ( (found_p_ncc > 1) and (len(prog_list) > 0) and (ct < my_config.LIMIT_ROUND) ):
        print("search round: ", ct, " - query_pmid #: ", len(prog_list)," - current component #: ", n_cluster_p, " - result component #: ", found_p_ncc)
    ...
    while ( (found_d_ncc > 1) and (len(dec_list) > 0) and (ct < my_config.LIMIT_ROUND) ):
        print("search round: ", ct, " - query_pmid #: ", len(dec_list)," - current component #: ", n_cluster_d, " - result component #: ", found_d_ncc)
    ...
    with open(my_config.LOG_PATH, 'a+') as log_fd:
        print(','.join(paper_list), " --> ", my_config.RESULT_PDF_PATH, file=log_fd)
